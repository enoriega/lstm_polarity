...
def main(input_path):
    ...
    ix, rix = build_vocabulary(list(it.chain.from_iterable(i.tokens for i in instances)))
    ...
    builder = dy.BiRNNBuilder(NUM_LAYERS, WEM_DIMENSIONS, HIDDEN_DIM, params, dy.LSTMBuilder)
    ...
    for e in range(epochs):
        ...
        for i, instance in enumerate(training):
            ...
            prediction = run_instance(instance, builder, wemb, ix, W, V, b, HIDDEN_DIM, W_a, b_a, W_a_2, b_a_2)
        ...
        if sum(testing_classifications) >= 1:
            ...
            print(report)
        if avg_loss <= 3e-3:
            np.savetxt('Segment_2_Attention_2L_biLSTM.csv', result_save, delimiter=',')
            break
        print()
    np.savetxt('Segment_2_Attention_2L_biLSTM.csv', result_save, delimiter=',')
...
def output_attention_low(outputs, W_a, b_a):   # low-level attention (within each LSTM)
    outputs = dy.concatenate(outputs, d=1)
    a = W_a * outputs + b_a
    s = dy.transpose(dy.softmax(dy.transpose(a)))
    
    return dy.reshape(s*dy.transpose(outputs), d=(20,))

def output_attention_high(collected_vectors, W_a_2, b_a_2):   #high-level attention
...
def run_instance(instance, builder, wemb, ix, W, V, b, HIDDEN_DIM, W_a, b_a, W_a_2, b_a_2):
    ...
    for segment in instance.get_segments():
        ...
        if len(segment) > 0:
            ...
            selected_att = output_attention_low(outputs, W_a, b_a)
    ...
    lstm_result_att = output_attention_high(collected_vectors, W_a_2, b_a_2)
