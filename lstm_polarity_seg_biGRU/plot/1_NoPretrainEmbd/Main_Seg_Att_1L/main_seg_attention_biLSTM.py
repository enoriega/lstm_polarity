...
def main(input_path):
    ...
    for e in range(epochs):
        ...
        for i, instance in enumerate(training):
            ...
            prediction = run_instance(instance, builder, wemb, ix, W, V, b, HIDDEN_DIM, W_a, b_a)
        ...
        if sum(testing_classifications) >= 1:
            ...
            print(report)
        if avg_loss <= 3e-3:
            np.savetxt('Segment_2_Attention_1L_biLSTM.csv', result_save, delimiter=',')
            break
        print()
    np.savetxt('Segment_2_Attention_1L_biLSTM.csv', result_save, delimiter=',')
...
def run_instance(instance, builder, wemb, ix, W, V, b, HIDDEN_DIM, W_a, b_a):
