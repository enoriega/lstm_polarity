...
def main(input_path):
    ...
    # Training loop
    trainer = dy.SimpleSGDTrainer(params, learning_rate=0.001)
    ...
    for e in range(epochs):
        ...
        for i, instance in enumerate(training):
            ...
            prediction = run_instance(instance, builder, ix, W, V, b, HIDDEN_DIM)
        ...
        if sum(testing_classifications) >= 1:
            ...
            print(report)
        if avg_loss <= 3e-3:
            np.savetxt('Segment_1_No_Attention_biLSTM.csv', result_save, delimiter=',')
            break
        print()
    np.savetxt('Segment_1_No_Attention_biLSTM.csv', result_save, delimiter=',')
...
def run_instance(instance, builder, ix, W, V, b, HIDDEN_DIM):
